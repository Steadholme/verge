...
//! counter) when the queue is full or the worker is gone, so a slow/down Watchtower NEVER
//! blocks, slows, or fails a request. When audit is disabled (the dev/test default) the sink is
//! a no-op — no channel, no worker.
//!
//! A device PRIVATE KEY never rides an event: producers fill only the logical fields (actor /
//! action / target / severity / detail), and `detail` is kept short and value-free at each call
//! site. The enroll/revoke audit records WHO touched WHICH device WHEN — never key material.
...
impl AuditSink {
    ...
    /// Emit one event. Sync, non-blocking, infallible: `try_send` only; on a full queue or a
    /// dead worker the event is DROPPED (drop counter + warn). NEVER blocks or errors the
    /// request path.
...
mod tests {
    ...
    fn enroll_serializes_without_key_material() {
        ...
        assert_eq!(v["action"], "mycelium.enroll");
        assert_eq!(v["source"], "mycelium");
        // No base64 private key shape rides along.
