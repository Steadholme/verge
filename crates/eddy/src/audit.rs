...
//! worker.
//!
//! Only the logical fields ride an event (actor / action / target=path / severity / detail); no
//! asset bytes are ever carried. The notable event is `eddy.purge` — WHO invalidated WHICH path or
//! hash, WHEN.
...
impl AuditEvent {
    ...
    /// `info`-severity event (normal lifecycle: an asset cached).
    ...
    /// `warning`-severity event (destructive: cache purge).
...
mod tests {
    ...
    #[test]
    fn purge_serializes_to_safe_fields() {
        let ev = AuditEvent::warning("eddy.purge", "alice@w33d.xyz", "css/app.css", "by path");
        ...
        assert_eq!(v["action"], "eddy.purge");
        assert_eq!(v["source"], "eddy");
    }
    ...
    async fn emit_never_blocks_when_sink_unreachable() {
        ...
        for _ in 0..(QUEUE_CAPACITY * 8) {
            sink.emit(AuditEvent::warning("eddy.purge", "u", "p", "d"));
        }
        assert!(
            sink.dropped() > 0,
